...
public:
    ...
    // 速度就定成speed
    // 返回吃完所有的香蕉，耗费的小时数量
    static long long f(const std::vector<int>& piles, int speed) {
        long long ans = 0;
        for (int pile : piles) {
...
int main() {
    ...
    // 调用minEatingSpeed函数
    int result = okoEatingBananas::minEatingSpeed(piles, h);

    // 输出结果
    std::cout << "The minimum eating speed to finish all bananas within " << h << " hours is: " << result << std::endl;
